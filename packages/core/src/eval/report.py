...
from upmixer.eval.report_format import format_report as format_report
...
class EvalReport:
    ...
    def recording_means(self) -> list[dict[str, object]]:
        ...
        groups: dict[tuple[str, str, str, str | None], list[StemScore]] = defaultdict(
            list
        )
        for score in score_map.values():
            groups[
                (score.recording_id, score.category, score.stem, score.split)
            ].append(score)

        rows: list[dict[str, object]] = []
        for (recording_id, category, stem, split), scores in sorted(
    ...
    def to_dict(
        self, *, paired_bootstrap: dict[str, object] | None = None
    ) -> dict[str, object]:
    ...
    def paired_bootstrap(
        ...
        *,
        ...
        if len({key[0] for key in matched_keys}) < 2:
            raise ValueError(
                "paired bootstrap requires at least two matched recordings"
            )
        ...
        confidence_intervals = {
            ...
            "by_stem": _bootstrap_groups(
                left, right, matched_keys, 3, n_resamples, confidence, rng
            ),
            "by_category": _bootstrap_groups(
                left, right, matched_keys, 2, n_resamples, confidence, rng
            ),
        }
        return {
            "pairs": pairs,
...
def _named_means(
    ...
    return {
        group: {metric: float(values[index]) for index, metric in enumerate(_METRICS)}
...
def _validated_score_map(scores: list[StemScore]) -> dict[_ScoreKey, StemScore]:
    ...
    for score in scores:
        if (
            not isinstance(score.recording_id, str)
            or not score.recording_id.strip()
            or not isinstance(score.item_id, str)
            or not score.item_id.strip()
        ):
            raise ValueError(
                "paired comparison requires stable recording_id and item_id"
            )
        values = []
...
def _paired_rows(
    ...
    groups: dict[
        tuple[str, str, str, str | None], list[tuple[StemScore, StemScore]]
    ] = defaultdict(list)
    for key in matched_keys:
        groups[(key[0], key[2], key[3], key[4])].append((left[key], right[key]))

    rows: list[dict[str, object]] = []
    for (recording_id, category, stem, split), score_pairs in sorted(
        groups.items(), key=lambda item: _sort_key(item[0])
    ):
...
def _validated_coverage_map(rows: list[CoverageRow]) -> dict[_ScoreKey, CoverageRow]:
    ...
    for row in rows:
        if (
            not isinstance(row.recording_id, str)
            or not row.recording_id.strip()
...
def _paired_coverage(
    ...
    unavailable = [
        ...
        for key, row in sorted(
            left_coverage.items(), key=lambda item: _sort_key(item[0])
        )
    ...
    unavailable.extend(
        ...
        for key, row in sorted(
            right_coverage.items(), key=lambda item: _sort_key(item[0])
        )
        if row.status != "scored"
    )
    return {
        ...
        "left_only_recordings": sorted(
            {key[0] for key in left_all_keys} - {key[0] for key in right_all_keys}
        ),
        "right_only_recordings": sorted(
            {key[0] for key in right_all_keys} - {key[0] for key in left_all_keys}
        ),
...
def _paired_recording_deltas(
    ...
    groups: dict[tuple[str, str | None, str], list[tuple[float, float, float]]] = (
        defaultdict(list)
    )
    for key in matched_keys:
    ...
    by_group: dict[tuple[str | None, str], dict[str, tuple[float, float, float]]] = (
        defaultdict(dict)
    )
    for (recording_id, split, group), deltas in groups.items():
        means = tuple(
            float(np.mean([delta[index] for delta in deltas])) for index in range(3)
        )
...
def _bootstrap_groups(
    ...
    for (split, group), recording_values in sorted(
        grouped.items(), key=lambda item: _sort_key(item[0])
    ):
        recording_ids = sorted(recording_values)
        values = np.asarray(
            [recording_values[recording_id] for recording_id in recording_ids],
            dtype=float,
        )
        ...
        if len(recording_ids) >= 2:
            sample_indices = rng.integers(
                0, len(recording_ids), size=(n_resamples, len(recording_ids))
            )
            sample_means = values[sample_indices].mean(axis=1)
            bounds = np.quantile(
                sample_means, (tail, 1.0 - tail), axis=0, method="linear"
            )
        else:
            bounds = None
        group_result.update(
...
def _validate_bootstrap_args(n_resamples: int, confidence: float, seed: int) -> None:
    ...
    if (
        not isinstance(confidence, (int, float, np.integer, np.floating))
