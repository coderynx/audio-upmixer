...
| # | Difference | Bound |
|---|---|---|
| P1 | **Live-parameter latency.** The preview renders ahead of the playhead, so a control change lands after the render horizon rather than instantly. The horizon is whichever zero-phase stage is active and reaches furthest: the LF unifier's 100 ms, or mid-bass decorrelation's 300 ms (its 4th-order 100-300 Hz band-pass needs the longer context — see `spatial_layouts_bs775_bs2051.md`). The native Apple media stream queues up to 16384 frames (341.3 ms) ahead of its presentation clock, with 2048 frames (42.7 ms) of startup prefill. | Browser/direct: ~100 ms, or ~300 ms with `mastering_bass_decorrelate` above 0; Apple Spatial: up to another 341.3 ms of submitted PCM, plus device/system output latency |
...
| P7 | **Apple Spatial monitor render.** The Tauri-only Apple Spatial mode hands every multichannel mastered speaker bed to `AVSampleBufferAudioRenderer` with a standard channel layout and sample-based timestamps; stereo uses direct `AVAudioEngine` output. The adapter preserves programme PCM including LFE, reorders 7.1.4 side/rear channels, and applies only transport monitor volume. The former PHASE LFE fold and -3 dB trim are removed. System spatialization and head tracking remain monitor-only and are controlled by macOS. Loudness measures pre-renderer PCM and does not claim final spatialized acoustic loudness or exact Apple Music/Logic parity. | Pre-renderer programme remains algorithm-identical before transport volume; spatialized output and its unmeasured final loudness are not an export reference |
